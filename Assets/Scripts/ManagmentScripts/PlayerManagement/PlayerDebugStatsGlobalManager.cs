...
public class PlayerDebugStatsGlobalManager : MonoBehaviour
{
    ...
    public string playerStatsDataName = "/playerStats.txt";
    ...
    private void Awake()
    {
        ...
        {
            Instance = this;
            // removes main menu and end menu
            levelCount = SceneManager.sceneCountInBuildSettings - 2;
            //DontDestroyOnLoad(gameObject);
            playerStatsDataName = HTMLPlatformUtil.GetSavePath() + "/playerStatsData.txt";
            Load(); // Load on start
        }
        else
        {
    ...
    public void Save()
    {
        ...
        if (dataLocal == null)
        {
            ...
            dataLocalToSave = new PlayerDebugStatsGlobal
            {
                ...
                amountPlayerJumps = 0,
                hasBeatenGame = false,
				fastestLevelTimes = Enumerable.Repeat(0f, levelCount).ToList(),
				currentLevelTimes = Enumerable.Repeat(0f, levelCount).ToList(),
        ...
        {
            File.WriteAllText(playerStatsDataName, json);
        }
    }
    ...
    public void Load()
    {
        ...
        {
            if (!File.Exists(playerStatsDataName))
            {
            ...
            json = File.ReadAllText(playerStatsDataName);
        }
    ...
    public void DeleteSave()
    {
        ...
        else
        {
            if (File.Exists(playerStatsDataName))
            {
                File.Delete(playerStatsDataName);
            }
        }
